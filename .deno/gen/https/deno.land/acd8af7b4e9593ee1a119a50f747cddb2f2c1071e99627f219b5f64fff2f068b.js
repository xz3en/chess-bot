...
export class Application extends EventTarget {
   ...
   * response. */ // deno-lint-ignore no-explicit-any
    ...
    /** Processing registered middleware on each request. */ async #handleRequest(request, secure, state) {
        let context1;
        try {
            context1 = new Context(this, request, this.#getContextState(), {
                secure,
                ...this.#contextOptions
            });
        } catch (e) {
            const error1 = e instanceof Error ? e : new Error(`non-error thrown: ${JSON.stringify(e)}`);
            const { message: message1  } = error1;
            this.dispatchEvent(new ApplicationErrorEvent({
                message: message1,
                error: error1
            }));
        ...
        assert(context1, "Context was not created.");
        let resolve;
